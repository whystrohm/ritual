...
- **`docs/how-routines-work.md`** — full walkthrough of Claude Code scheduled triggers. Covers remote cloud execution, three execution contexts (trigger / GitHub Actions / local cron), exact 4-click path to create a trigger, test-before-trust pattern, usage limits per plan, troubleshooting. Critical for new users who have never built a Claude Code routine before.
- Bootstrap Phase 4 now classifies each recommendation by execution context (claude-code-trigger, github-actions, local-cron-or-launchd) and emits explicit "Next 4 clicks" instructions for trigger-ready patterns. Honest scoping: if a pattern belongs in GitHub Actions or launchd, the scan says so and does not force a trigger fit.
- `docs/first-routines.md` overhauled — seven archetypes classified by execution context. Archetype 1 (voice sweep) and 2 (fact freshness) remain Claude Code triggers; added Archetype 3 (dependency + security digest), 4 (content calendar sync via Notion/Drive MCP), 5 (Gmail inbox triage). Archetype 6 (deploy verification) explicitly labeled as GitHub Actions, Archetype 7 (local filesystem) explicitly labeled as launchd — both out of scope for Claude Code triggers.
- README repositioned — bootstrap scan is the unambiguous hero; voice-lint is demoted to "Archetype 1 / the bundled skill that powers it."
- Badge changed: "Scheduled Triggers: ready" linking to `claude.ai/code/scheduled` (verified URL) instead of the old unverified docs URL.
